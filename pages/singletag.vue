<template>
  <div>
    <head-banner class="little_text_banner" :title="$t('message.general_video_games')+' <span class=\'yellow_link\'> '+displayed_tag.tag_name+'</span>'"></head-banner>
                        ...
                        <v-container fluid>
                          <v-row class="search_game_filer_section mb-1">
                            ...
                            <v-col cols="4" sm='4' md='2' class="pb-0 bg_blue_class">
                              <v-subheader class="mb-0 pl-0 pb-1 slider_subheader">  {{$t('message.order_by') }}  </v-subheader>
                                <v-select
                          ...
                          <v-row>
                            <list-of-games  :key="listFilterKey" rand="0" :ordering="selectedOrder.value"  show_title="0" :title="selectedTitle" :tag_id="displayed_tag.id_dDblKey" limit="20" :note_min="selectedRating" nombre_vote="0" no-ssr/></list-of-games>
                          </v-row>
                        </v-container>
                      </div>
                    </div>
...
<script lang="js">
  ...
  export default  {
    name: 'SingleTag',
    props: ['tagid', 'used_lang_for_route', 'locale'],
    data () {
      return {
        displayed_tag:'',
        loaded:false,      
    ...
    head() {
        return {
            title: 'DeusSearch - Jeux-vidéo '+ this.displayed_tag.tag_name,
            bodyAttrs: {
    ...
    async asyncData({ $axios, params}) {
      const tag = await $axios.get('https://api.deussearch.fr/get_single_tag.php?&tid='+params.tagid);
      const displayed_tag = tag.data[0];
      return {
       displayed_tag
      }
    }

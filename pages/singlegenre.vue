<template>
  <div>
    <head-banner class="little_text_banner" :title="$t('message.general_video_games')+' <span class=\'yellow_link\'> '+displayed_genre.name_genre+'</span>'"></head-banner>
                    ...
                    <div class="contact-area text_page p-0">
                      <div>
                        <v-container fluid>
                          ...
                          <v-row>
                            <list-of-games :key="listFilterKey" :ordering="selectedOrder.value" rand="0" :genre_id="displayed_genre.id_dDblKey" show_title="0" :title="selectedTitle" limit="20" :note_min="selectedRating" nombre_vote="0" no-ssr/></list-of-games>
                          </v-row>
                        </v-container>
                      </div>
                    </div>
...
<script lang="js">
  ...
  export default  {
    name: 'SingleGenre',
    props: ['genreid', 'used_lang_for_route', 'locale'],
    data () {
      return {
        displayed_genre:'',
        loaded:false ,      
    ...
    head() {
        return {
            title: 'DeusSearch - Jeux-vidéo '+ this.displayed_genre.name_genre,
            bodyAttrs: {
    ...
    async asyncData({ $axios, params}) {
      const genre = await $axios.get('https://api.deussearch.fr/get_single_genre.php?&gid='+params.genreid);
      const displayed_genre = genre.data[0];
      return {
       displayed_genre
      }
    }
